...
function test_01( diag_method::String )
    ...
    if diag_method == "LOBPCG"
        @time evals = diag_LOBPCG!(
            Ham, psi, verbose=false, verbose_last=true
        )
    ...
    elseif diag_method == "PCG"
        ...
        error( @sprintf("ERROR: unknown diag_method = %s\n", diag_method) )
    end
...
function test_02( diag_method::String )
    ...
    if diag_method == "LOBPCG"
        @time evals = diag_LOBPCG!( Ham, psi, verbose=false )
    ...
    elseif diag_method == "PCG"
        ...
        error("Unknown diag_method = ", diag_method)
    end
