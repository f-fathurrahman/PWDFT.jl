...
function test_no_spin(kT::Float64)
    ...
    Focc_v1, E_fermi_v2 = calc_Focc( evals, wk, Nelectrons, kT, Nspin=Nspin)
    @printf("E_fermi = %18.10f\n", E_fermi_v2)

    E_fermi_v2 = efermig( Nelectrons, wk, kT, evals, Nspin )
    @printf("E_fermi_v2 = %18.10f\n", E_fermi_v2)
    Focc_v2 = calc_Focc_v2( wk, kT, evals, E_fermi_v2, Nspin )

    for ik = 1:Nkpt
        @printf("\n")
        for ist = 1:Nstates
            @printf("[ik=%3d,ist=%3d]: %18.10f %18.10f %18.10f\n",
                     ik, ist, evals[ist,ik], Focc_v1[ist,ik], Focc_v2[ist,ik])
        end
    end
    
    @printf("sum(Focc_v1) = %18.10f\n", sum(Focc_v1)/Nkpt)
    @printf("sum(Focc_v2) = %18.10f\n", sum(Focc_v2)/Nkpt)
