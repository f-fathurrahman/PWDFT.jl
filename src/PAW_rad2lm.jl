function PAW_rad2lm!(
    ...
    atoms, pspotNL,
    lmax_loc::Int64,
    F_rad::Vector{Matrix{Float64}}, F_lm
)
    # Computes:
    ...
    Nspin = size(F_rad[1], 2)
    Nrmesh = size(F_rad[1], 1)
    ...
    for ispin in 1:Nspin, lm in 1:lmax_loc^2
        ...
        for ix in 1:sphere.nx, j in 1:Nrmesh
            F_lm[j,lm,ispin] += F_rad[ix][j,ispin] * sphere.wwylm[ix,lm]
        end
    end
...
function PAW_rad2lm3!(
    ...
    atoms, pspotNL,
    lmax_loc::Int64,
    F_rad::Vector{Array{Float64,3}}, F_lm
)
    # 2nd dim is direction: (x,y,z) or (r,ϕ,θ)

    Nspin = size(F_rad[1], 3)
    Nrmesh = size(F_rad[1], 1)
    isp = atoms.atm2species[ia]
    sphere = pspotNL.paw.spheres[isp]
    ...
    for ispin in 1:Nspin, lm in 1:lmax_loc^2
        @views F_lm[:,:,lm,ispin] .= 0.0
        for ix in 1:sphere.nx, j in 1:Nrmesh
            F_lm[j,1,lm,ispin] += F_rad[ix][j,1,ispin] * sphere.wwylm[ix,lm]
            F_lm[j,2,lm,ispin] += F_rad[ix][j,2,ispin] * sphere.wwylm[ix,lm]
            F_lm[j,3,lm,ispin] += F_rad[ix][j,3,ispin] * sphere.wwylm[ix,lm]
        end
    end
