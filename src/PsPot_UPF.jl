...
function PsPot_UPF( upf_file::String )
    ...
    # XXX: Data generated by atompaw seems to have wrong mesh_size information in the header
    # XXX: This might cause error in an assert statement below (comparing that
    # XXX: length(spl_str) is equal to Nr
    # XXX: I solved this problem by modifying the UPF file manually.
    ...
    # In some cases `Nr != length(spl_str)` (ex: PAW JTH dataset)
    # Nr is read from header, `spl_str` is obtained from the actual length
    # of splitted strings. To avoid hassle we might want to adjust Nr
    # so that is equal to length of `spl_str`
    if Nr != length(spl_str)
        println()
        println("!!! WARNING: Nr value from pseudopotential header is")
        println("!!! WARNING: different from length(spl_str).")
        println("!!! WARNING: Nr will be set to length(spl_str)")
        println()
        Nr = length(spl_str)
    end
